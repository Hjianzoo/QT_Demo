...
/*检查6个兴趣爱好复选框，根据兴趣复选框状态，更新三态复选框显示*/
...
void Widget::on_checkBoxTristate_toggled(bool checked)
{
    if(ui->checkBoxTristate->checkState() == Qt::Unchecked)
    {
        ui->checkBoxRead->setChecked(0);
        ui->checkBoxClimb->setChecked(0);
        ui->checkBoxSwim->setChecked(0);
        ui->checkBoxDaze->setChecked(0);
        ui->checkBoxShopping->setChecked(0);
        ui->checkBoxEat->setChecked(0);
        qDebug()<<tr("全部取消");
    }
}
...
{
    QDesktopServices::openUrl(QUrl("file:///D:Hjian"));
}
